...
public class SiteSession {
    ...
    private static ThreadLocal<Long> globalLastHitMillis = new ThreadLocal<Long>(){
        @Override
     ...
     *
     * @throws java.lang.IllegalArgumentException if the time is less than the global max
    ...
    public void update(long hitMillis, String url) {
        ...
        if(lastHitMillis > 0 && lastHitMillis+MAX_IDLE_MS < hitMillis) {
            System.out.println("HIT: " + hitMillis);
            System.out.println("PREVIOUS: " + lastHitMillis);
            System.out.println("GLOBAL: " + globalLastHitMillis.get());
            throw new IllegalArgumentException("interval since last hit exceeds session timeout");
        }
        this.lastHitMillis = hitMillis;

        if(hitMillis < globalLastHitMillis.get()) {
            throw new IllegalArgumentException("hit processed out of order");
        }
        globalLastHitMillis.set(hitMillis);
    ...
    public boolean isExpired() {
        return globalLastHitMillis.get()-lastHitMillis > MAX_IDLE_MS;
    }
    public boolean isExpired(long localTime) {
        return localTime-lastHitMillis > MAX_IDLE_MS;
    }
    ...
    public static void resetGlobalMax() {
        globalLastHitMillis.set(new Long(0));
    }
