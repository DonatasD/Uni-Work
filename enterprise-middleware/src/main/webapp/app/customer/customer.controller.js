...
(function() {
	'use strict';
	angular.module('app.customer').controller('CustomerController',
			CustomerController);

	CustomerController.$inject = [ '$scope', '$routeParams', '$location',
			'Customer', 'messageBag' ];

	function CustomerController($scope, $routeParams, $location, Customer,
			messageBag) {
		// Assign Customer service to $scope
		...
		// If $routeParams has :customerId then load the specified customer, and
		// display edit controls on customerForm
		...
		// Define a reset function, that clears the prototype new Customer
		...
		// Define an addCustomer() function, which creates a new customer via
		// the
		// REST service,
		...
		// Define a saveCustomer() function, which saves the current customer
		...
		$scope.saveCustomer = function() {
			...
			function(data) {
				...
				var idx = _.findIndex($scope.customers.data, {
