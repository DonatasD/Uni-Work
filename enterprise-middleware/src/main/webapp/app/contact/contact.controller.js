...
(function() {
	'use strict';
	angular.module('app.contact').controller('ContactController',
			ContactController);

	ContactController.$inject = [ '$scope', '$routeParams', '$location',
			'Contact', 'messageBag' ];
	...
	function ContactController($scope, $routeParams, $location, Contact,
		...
		// If $routeParams has :contactId then load the specified contact, and
		// display edit controls on contactForm
		if ($routeParams.hasOwnProperty('contactId')) {
			$scope.contact = $scope.contacts.get({
				contactId : $routeParams.contactId
			});
			$scope.create = false;
		}

		// Define a reset function, that clears the prototype new Contact
		// object, and
		...
		// Define an addContact() function, which creates a new contact via the
		// REST service,
		...
		// Define a saveContact() function, which saves the current contact
		...
		$scope.saveContact = function() {
			...
			function(data) {
				...
				var idx = _.findIndex($scope.contacts.data, {
					'id' : $scope.contact.id
				});
		...
		// Define a deleteContact() function, which saves the current contact
		// using the REST service
		// and displays any error messages
		$scope.deleteContact = function() {
			...
			// Successful query
			function() {
				// TODO: Fix the wonky imitation of a cache by replacing with a
				// proper $cacheFactory cache.
